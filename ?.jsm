...
sock.sendMessage = async (jid, content, options = {}) => {
    try {
        ...
        if (isMediaMessage && !options.skipVerified) {
            // Only add verified badge context, NO newsletter forwarding
            content.contextInfo = {
                ...(content.contextInfo || {}),
                forwardingScore: 999,
                isForwarded: true,
            ...
            // Add quoted message for verified badge display (without newsletter)
            if (!options.quoted) {
                options.quoted = {
                    key: {
                        remoteJid: "0@s.whatsapp.net",
                    ...
                    message: {
                        conversation: "```𓂋⃝⃟⃟⃝⃪⃔ CODY AI⃝⃪⃔⃕```"
                    }
                };
            }
        }
